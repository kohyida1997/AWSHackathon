...
def clean(csvFile):
    ...
    for col_name in required_cols:
        if col_name not in columns:
            raise RuntimeError(f"field {col_name} not found in csv")
...
if __name__ == "__main__":
    filename = sys.argv[1]
    seed = int(sys.argv[2])
    lines_to_pick = int(sys.argv[3])

    import random
    random.seed(seed)

    with open(f"clean/{filename}", encoding='utf-8-sig') as in_file:
        rows = list(csv.reader(in_file))
        header_row = rows[0]
        timestamp_idx = header_row.index("timestamp")
        id_idx = header_row.index("id")
        rows = rows[1:] # remove header row
        unempty_rows = []
        for row in rows:
            if is_ok(row):
                unempty_rows.append(row)
        n = len(unempty_rows)
        if lines_to_pick > n:
            raise RuntimeError("trying to pick more rows than available")
        random.shuffle(unempty_rows)

        new_rows = [",".join(header_row)]
        for i in range(lines_to_pick):
            row = unempty_rows[i]
